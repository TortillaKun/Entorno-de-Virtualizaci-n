...
function Verificar-DHCP {
    ...
    if ($feature.Installed) { Write-Host "DHCP instalado" } else { Write-Host "DHCP no instalado" }
}
...
function Configurar-DHCP {
    ...
    if (-not $adapter) { Write-Host "No se encontro Ethernet 2"; return }
    if ($adapter.Status -ne "Up") { Write-Host "Ethernet 2 no esta activa"; return }

    do {
        $IP_INICIAL = Read-Host "IP inicial (sera IP del servidor)"
        if (-not (Validar-IP $IP_INICIAL)) { Write-Host "IP invalida ejemplo 192.168.50.50" }
    } until (Validar-IP $IP_INICIAL)

    do {
        $IP_FINAL = Read-Host "IP final"
        if (-not (Validar-IP $IP_FINAL)) { Write-Host "IP invalida ejemplo 192.168.50.60" }
    } until (Validar-IP $IP_FINAL)

    if (-not (Misma-Red $IP_INICIAL $IP_FINAL)) { Write-Host "Error no pertenecen a la misma red"; return }
    if (-not (Validar-Rango $IP_INICIAL $IP_FINAL)) { Write-Host "Rango invalido"; return }

    $IP_SERVIDOR = $IP_INICIAL
    $IP_RANGO_INICIAL = Sumar-Uno $IP_INICIAL

    if (-not (Validar-Rango $IP_RANGO_INICIAL $IP_FINAL)) { Write-Host "Error el servidor ocupa todo el rango"; return }
    ...
    Set-DhcpServerv4OptionValue -ScopeId $red -Router $IP_SERVIDOR -DnsServer $IP_SERVIDOR | Out-Null

    Set-DhcpServerv4Scope -ScopeId $red -State Active | Out-Null

    Restart-Service DHCPServer

    Set-DnsClientServerAddress -InterfaceAlias "Ethernet 2" -ServerAddresses $IP_SERVIDOR
...
function Monitoreo {
    ...
    if ($scopes) {
        foreach ($scope in $scopes) {
            ...
            Get-DhcpServerv4Lease -ScopeId $scope.ScopeId | Select-Object IPAddress, HostName, ClientId, AddressState
        }
